...
if ($row) {
    $obj = new stdClass();
    $obj->code = "2";
    $obj->msg = urlencode($langs['already_registered']); //中文urlencode一下
    echo urldecode(json_encode($obj));
    return;
}
...
if (insert("dfz_user", $arr) && insert("dfz_userinfo", $arr2)) {
    $obj = new stdClass();
    $obj->code = "0";
    $obj->msg = urlencode($langs['created_success']); //中文urlencode一下
    echo urldecode(json_encode($obj));
} else {
    //回溯
    ...
    $obj = new stdClass();
    $obj->code = "1";
    $obj->msg = urlencode($langs['creation_failed']); //中文urlencode一下
    echo urldecode(json_encode($obj));
}
